...
function creationPanier()
{
   ...
   if (!isset($_SESSION['panier']))
   {
      ...
      $_SESSION['panier']['numeroCommande'] = array();
...
function ajouterArticle($codeProduit,$quantite,$numeroCommande)
{
   //si le panier existe
   if (creationPanier() && !isVerrouille())
   {
      ...
      else
      {
         ...
         array_push( $_SESSION['panier']['numeroCommande'],$numeroCommande);
      }
   }
   ...
   if (creationPanier() && !isVerrouille())
   {
      ...
      for($i = 0; $i < count($_SESSION['panier']['codeProduit']); $i++)
      {
         if ($_SESSION['panier']['codeProduit'][$i] !== $codeProduit)
         {
            ...
            array_push( $tmp['numeroCommande'],$_SESSION['panier']['numeroCommande'][$i]);
         }
...
function modifierQTeArticle($codeProduit,$quantite)
{
   //si le panier existe
   if (creationPanier() && !isVerrouille())
   {
...
function MontantGlobal()
{
   ...
   for($i = 0; $i < count($_SESSION['panier']['codeProduit']); $i++)
   {
      $total += $_SESSION['panier']['quantite'][$i] * $_SESSION['panier']['numeroCommande'][$i];
   }
